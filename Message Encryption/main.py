# Imported libraries essential to the building of the project
from tkinter import *
import base64
...
# Defining the function that encodes the message
def encode(key, msg):
    enc = []
...
# Defining the function that decodes the message
def decode(key, code):
    dec = []
...
# Creating the Graphic User Interface (GUI) with tkinter
wn = Tk()
wn.geometry("500x500")
wn.configure(bg='azure2')
wn.title("Encrypt and Decrypt your Messages with HecEncrypt")
Message = StringVar()
key = StringVar()
mode = IntVar()
Output = StringVar()

# Creating individual properties for the GUI
headingFrame1 = Frame(wn, bg="gray91", bd=5)
headingFrame1.place(relx=0.2, rely=0.1, relwidth=0.7, relheight=0.16)

headingLabel = Label(headingFrame1, text=" Welcome to Encryption and \nDecryption with Group 5", fg='grey19',font=('Courier', 15, 'bold'))
...
def Result():
    ...
    if i == 1:
        ...
        messagebox.showinfo('HecEncrypt', 'Please Choose one of Encryption or Decryption. Try again.')
...
#  Building the result, exit and reset buttons
